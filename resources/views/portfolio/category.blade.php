...
@section('content')
    <section class="bg-neutral-950 text-neutral-200 py-20 px-8 text-center border-b border-neutral-800">
        <div class="max-w-[1400px] mx-auto">
            ...
            <p class="text-[1.05rem] opacity-80 max-w-[600px] mx-auto tracking-wide">Explore my {{ $category }} designs
                and projects</p>
        </div>
    </section>
    ...
    <section class="flex gap-3 p-8 justify-center flex-wrap max-w-[1400px] mx-auto border-b border-neutral-800">
        <a href="{{ route('portfolio.index') }}"
            class="bg-transparent border border-neutral-800 py-2 px-5 rounded-sm cursor-pointer font-medium transition-all duration-200 text-neutral-400 text-sm tracking-wide capitalize hover:border-neutral-600 hover:text-neutral-200">All</a>
        <a href="{{ route('portfolio.category', $category) }}"
            class="bg-white text-neutral-950 border border-white py-2 px-5 rounded-sm cursor-pointer font-medium transition-all duration-200 text-sm tracking-wide capitalize">{{ $category }}</a>
    </section>
    ...
    <div class="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-6 p-12 max-w-[1400px] mx-auto">
        @forelse ($designs as $design)
            <div
                ...
                <div class="p-6">
                    <div
                        class="inline-block bg-neutral-800 text-neutral-400 py-1.5 px-3 rounded-sm text-xs mb-3 uppercase tracking-wider font-medium">
                        {{ $design->category }}</div>
                    <h3 class="text-base font-semibold mb-2 text-white tracking-wide">{{ $design->title }}</h3>
                    <p class="text-neutral-400 text-sm leading-relaxed mb-4">{{ Str::limit($design->description, 100) }}
                    </p>
                    <a href="{{ route('portfolio.show', $design) }}"
                        class="inline-block bg-transparent text-neutral-200 py-2 px-4 border border-neutral-800 rounded-sm no-underline text-sm font-medium transition-all duration-200 tracking-wide hover:bg-neutral-900 hover:border-neutral-600 hover:text-white">View
                        Project →</a>
                </div>
            </div>
        @empty
            <div class="col-span-full text-center p-12">
                ...
                <a href="{{ route('portfolio.index') }}"
                    class="inline-block bg-transparent text-neutral-200 py-2 px-4 border border-neutral-800 rounded-sm no-underline text-sm font-medium transition-all duration-200 tracking-wide hover:bg-neutral-900 hover:border-neutral-600 hover:text-white mt-4">Back
                    to Portfolio</a>
            </div>
        @endforelse
    </div>
